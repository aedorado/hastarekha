...
interface HandEntry {
  ...
  handType: string;
  handTattva: string;
  view: HandView | null; // null = placeholder (no images)
  viewLabel: string;
  imageUrl: string | null;
  isPlaceholder: boolean;
}

const VIEW_ORDER: HandView[] = ['right_palm', 'right_back', 'left_palm', 'left_back'];

function buildEntries(profiles: HandProfile[]): HandEntry[] {
  ...
  for (const profile of sorted) {
    ...
    const hasImages = VIEW_ORDER.some((v) => !!profile.images[v]);
      ...
      for (const view of VIEW_ORDER) {
        ...
        if (url) {
          entries.push({
            ...base,
            view,
            viewLabel: HAND_VIEW_LABELS[view],
            imageUrl: url,
            isPlaceholder: false,
          });
        }
...
export default function AllHandsView({ profiles, isLoading }: AllHandsViewProps) {
  ...
  // Build entries whenever profiles change
  useEffect(() => {
    const built = buildEntries(profiles);
    setEntries(built);
    setCurrentIndex(0);
  }, [profiles]);
          ...
          <div className="flex items-end justify-between gap-2">
            ...
            <div className="flex flex-col items-end gap-1.5 pointer-events-auto shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1 text-[10px] font-bold text-amber-300 hover:text-white bg-white/10 hover:bg-accent-gold rounded px-2 py-1 transition-all duration-150 border border-white/10 backdrop-blur-sm"
              >
