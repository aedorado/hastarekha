...
export default function AnalysisForm({
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-sm">
      {/* Save Action Bar */}
      <div className="p-4 bg-stone-50 border-b border-stone-200 flex items-center justify-between">
        <h3 className="mystic-title text-sm tracking-wider uppercase">Analysis Details</h3>
        <button
          onClick={onSave}
          disabled={isSaving || !profile.name}
          className="btn-gold px-4 py-2 text-xs flex items-center gap-1.5 shadow"
        >
      ...
      {/* Tabs */}
      <div className="flex bg-stone-50/50 border-b border-stone-200 text-xs overflow-x-auto">
        {(['profile', 'samudrika', 'mounts', 'lines', 'pins', 'tags'] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`flex-1 py-3 text-center font-semibold capitalize transition-all border-b-2 whitespace-nowrap px-3 ${
              activeTab === tab
